...
export default function CreateWorkOrderDetailsStepInspectionForm({
  inspectionForm,
  index,
}) {
  return (
      ...
      <AccordionItem>
        ...
        <AccordionPanel bg={"#fff7f7"} px={"0px"} py={"10px"}>
          <Flex flexDir={"column"} px={"20px"} py={"20px"}>
            ...
            !inspectionForm?.inspection_form?.status === "completed" ? (
              <Flex
                bg={"white"}
                py={"10px"}
                ...
                gap={"10px"}
              >
                {(inspectionForm?.inspection_questions?.length > 0
                  ? inspectionForm?.inspection_questions
                  : inspectionForm?.inspection_questions
              ...
              <Flex fontSize={"14px"} flexDir={"column"} w={"100%"}>
                ...
                {(inspectionForm?.inspection_questions?.length > 0
                  ? inspectionForm?.inspection_questions
                  : inspectionForm?.inspection_form?.inspection_questions
                )?.map((inspectionQuestion, inspectionQuestionIndex) => (
                  <Flex
                    w={"100%"}
                    color={"#848484"}
